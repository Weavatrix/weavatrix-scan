...
harness and publishes both the narrow-walker row and the equal path-plus-size
scanner result. Medians of three independent runs on a 20,000-file fixture:
paths-only lands at **0.94x** on both runtimes, close enough that the ordering
flips between runs, while the path-plus-size contract runs **80.1x** (Node) and
**86.7x** (Bun) faster because `fdir` needs one `statSync` per path. This npm
library is a separately released Scan product; it does not belong to Online or
MCP.
