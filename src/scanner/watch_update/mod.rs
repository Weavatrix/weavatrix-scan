...
pub(super) fn scan_watch_plan(
    ...
    let mut report = prepare_incremental_report(previous, plan);
...
pub(super) fn changed_candidate(
    ...
    let Some(metadata) = changed_metadata(&path, root, relative, options, report)? else {
...
fn local_error(
    ...
    if options.walk.error_policy == crate::walk_types::ErrorPolicy::Abort {
